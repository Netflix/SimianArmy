...
import com.netflix.simianarmy.aws.TestRDSRecorder;
...
public class TestRDSJanitorResourceTracker extends RDSJanitorResourceTracker {
    ...
    public void testInsertNewResource() {
        ...
        Assert.assertEquals(sqlCap.getValue(), "insert into janitortable (resourceId,resourceType,region,ownerEmail,description,state,terminationReason,expectedTerminationTime,actualTerminationTime,launchTime,markTime,dataJson) values (?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public void testUpdateResource() {
        ...
        List<Object> args = objCap.getAllValues();        
        Assert.assertEquals(sqlCap.getValue(), "update janitortable set resourceType=?,region=?,ownerEmail=?,description=?,state=?,terminationReason=?,expectedTerminationTime=?,actualTerminationTime=?,launchTime=?,markTime=?,dataJson=? where resourceId=?");
        Assert.assertEquals(args.size(), 12);
