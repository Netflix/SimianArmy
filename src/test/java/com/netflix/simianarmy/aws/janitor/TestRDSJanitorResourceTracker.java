...
public class TestRDSJanitorResourceTracker extends RDSJanitorResourceTracker {
    ...
    public void testInsertNewResource() {
        ...
        Assert.assertEquals(sqlCap.getValue(), "insert into janitortable (resourceId,resourceType,region,ownerEmail,description,state,terminationReason,expectedTerminationTime,actualTerminationTime,launchTime,markTime,additionalFields) values (?,?,?,?,?,?,?,?,?,?,?,?)");
        Assert.assertEquals(args.size(), 12);
        ...
        Assert.assertEquals(args.get(11).toString(), "{\"fieldName123\":\"fieldValue456\"}");
    }
    ...
    public void testUpdateResource() {
        ...
        List<Object> args = objCap.getAllValues();        
        Assert.assertEquals(sqlCap.getValue(), "update janitortable set resourceType=?,region=?,ownerEmail=?,description=?,state=?,terminationReason=?,expectedTerminationTime=?,actualTerminationTime=?,launchTime=?,markTime=?,additionalFields=? where resourceId=?");
        Assert.assertEquals(args.size(), 12);
        ...
        Assert.assertEquals(args.get(8).toString(), "0");
        Assert.assertEquals(args.get(9).toString(), markTime.getTime() + "");
        Assert.assertEquals(args.get(10).toString(), "{\"fieldName123\":\"fieldValue456\"}");
        Assert.assertEquals(args.get(11).toString(), id);
    }
