...
import com.netflix.simianarmy.aws.janitor.TestRDSJanitorResourceTracker;
...
public class TestRDSConformityClusterTracker extends RDSConformityClusterTracker {
    ...
    public void testInit() {        
        ...
        recorder.init();        
        Assert.assertEquals(sqlCap.getValue(), "create table if not exists janitortable ( cluster varchar(255), region varchar(25), ownerEmail varchar(255), isConforming varchar(10), isOptedOut varchar(10), updateTimestamp BIGINT, dataJson varchar(4096) )");
    }    
    ...
    public void testInsertNewCluster() {
        ...
        Assert.assertEquals(sqlCap.getValue(), "insert into janitortable (cluster,region,ownerEmail,isConforming,isOptedOut,updateTimestamp,dataJson) values (?,?,?,?,?,?,?)");
    ...
    public void testUpdateCluster() {
        ...
        List<Object> args = objCap.getAllValues();        
        Assert.assertEquals(sqlCap.getValue(), "update janitortable set ownerEmail=?,isConforming=?,isOptedOut=?,updateTimestamp=?,dataJson=? where cluster=? and region=?");
        Assert.assertEquals(args.size(), 7);
        ...
        Assert.assertEquals(args.get(5).toString(), "clustername1");
        Assert.assertEquals(args.get(6).toString(), "us-west-1");
    }
    ...
    public void testGetCluster() {
		...
		Assert.assertEquals(sqlCap.getValue(), "select * from janitortable where cluster = ? and region = ?");
    }
    ...
    public void testGetClusters() {
		...
		Assert.assertEquals(sqlCap.getValue().toString().trim(), "select * from janitortable where cluster is not null and region in ('us-west-1','us-west-2','us-east-1')");
    }
